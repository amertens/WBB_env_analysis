...
colnames(pr_wb)<-c("Location","TR", "N", "PR","lower.ci","upper.ci","Estimate", "SD","Robust SE","pval")
levels(pr_wb$TR)
...
#add bonferonni p-value
pr_wb$adjp<-pr_wb$pval*3
pr_wb$adjp<-ifelse(pr_wb$adjp>0.99,0.99,pr_wb$adjp)
...
colnames(pr_mid)<-c("Location","TR", "N", "PR","lower.ci","upper.ci","Estimate", "SD","Robust SE","pval")
levels(pr_mid$TR)
...
#add bonferonni p-value
pr_mid$adjp<-pr_mid$pval*3
pr_mid$adjp<-ifelse(pr_mid$adjp>0.99,0.99,pr_mid$adjp)
...
colnames(pr_end)<-c("Location","TR", "N", "PR","lower.ci","upper.ci","Estimate", "SD","Robust SE","pval")
levels(pr_end$TR)
...
#add bonferonni p-value
pr_end$adjp<-pr_end$pval*3
pr_end$adjp<-ifelse(pr_end$adjp>0.99,0.99,pr_end$adjp)
...
colnames(dif_wb)<-c("Location","TR", "N", "Dif","lower.ci","upper.ci", "SD","Robust SE", "pval")
...
#add bonferonni p-value
dif_wb$adjp<-dif_wb$pval*3
dif_wb$adjp<-ifelse(dif_wb$adjp>0.99,0.99,dif_wb$adjp)
...
colnames(dif_mid)<-c("Location","TR", "N", "Dif","lower.ci","upper.ci", "SD","Robust SE", "pval")
levels(dif_mid$TR)
...
#add bonferonni p-value
dif_mid$adjp<-dif_mid$pval*3
dif_mid$adjp<-ifelse(dif_mid$adjp>0.99,0.99,dif_mid$adjp)
...
colnames(dif_end)<-c("Location","TR", "N", "Dif","lower.ci","upper.ci", "SD","Robust SE", "pval")
...
#add bonferonni p-value
dif_end$adjp<-dif_end$pval*3
dif_end$adjp<-ifelse(dif_end$adjp>0.99,0.99,dif_end$adjp)
...
MidPts <- barplot(1:5,names.arg=NA,border=NA,col=NA,
  ...
  mtext(text=c("Geo. Mean=",d$log10),side=1,line=2.5,at=c(-2.5,pos),col=c(cblack,cols[which.arms]),cex=textsize,las=1)
...
# }
	...
	points(pos,d$PR,pch=21,cex=1.5,lwd=0,col=cols[which.arms],bg=alpha(cols[which.arms],alpha=0.5))
	  ...
	  # X-axis labels
		if(i==2 | i==11 | i==20){
  mtext(text=c("N=",d$N),side=1,line=1.5,at=c(-1,pos),col=c(cblack,cols[which.arms]),cex=textsize,las=1)
	mtext(text=c("P value=",d$pval),side=1,line=2.5,at=c(-1,pos),col=c(cblack,cols[which.arms]),cex=textsize,las=1)
	mtext(text=c("Bonferonni P value=",d$adjp),side=1,line=3.5,at=c(-1,pos),col=c(cblack,cols[which.arms]),cex=textsize,las=1)
...
if(i==1 | i==10 | i==19){
   op <- par(mar=c(4,0,2,1)+0.1)
	...
	if(i==1){ulabplot("E.coli log 10\ndifference \nat early \nassessment")}
	if(i==10){ulabplot("E.coli log 10\ndifference \nat one-year \nassessment")}
	if(i==19){ulabplot("E.coli log 10\ndifference \nat two-year \nassessment")}
...
pdf("Env Analysis- WB plot.pdf",width=10,height=8.5, paper="USr")
...
layout(m)
    ...
    diffplot(d=dif.dat[dif.dat$Location==levels(dif.dat$Location)[i[1]] & dif.dat$round==unique(dif.dat$round)[i[2]],], i[3])
    	...
    	segments(x0=grconvertX((2:8/9+.005)[c(1,2,5,6,7)], 'ndc', 'user'),y0=2.59, y1=4.02,lty=1,lwd=1,col="gray80")
    	segments(x0=grconvertX((2:8/9+.005)[c(1,2,3)], 'ndc', 'user'),y0=1.235, y1=2.38,lty=1,lwd=1,col="gray80")
    	segments(x0=grconvertX((2:8/9+.005)[c(1,2,3,4)], 'ndc', 'user'),y0= -.14, y1=1.02,lty=1,lwd=1,col="gray80")
